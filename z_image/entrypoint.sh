...
# mounted, so the Z-Image-Turbo weights survive across pod/worker restarts
...
# Default: Pod-based Celery worker, consuming the "z_image" queue from the
# shared Redis broker (see aafactory_nsfw's backend/celery_worker.py).
redis-server --protected-mode no &

uv run celery -A celery_worker.app worker --loglevel=info -Q z_image -P solo

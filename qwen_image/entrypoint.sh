...
# mounted, so model weights (Qwen-Image + Qwen-Image-Edit, both DFloat11
# checkpoints) survive across pod/worker restarts instead of being
# re-downloaded on every boot. Falls back to the default HF cache location
# when no volume is attached (e.g. local `docker run`, or a Pod without a
# volume configured).
...
# Default: Pod-based Celery worker, consuming the "qwen_image" queue from

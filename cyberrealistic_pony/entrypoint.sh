...
# mounted, so the CyberRealisticPony weights survive across pod/worker
...
# Default: Pod-based Celery worker, consuming the "cyberrealistic_pony"
# queue from the shared Redis broker (see aafactory_nsfw's
# backend/celery_worker.py).
redis-server --protected-mode no &

uv run celery -A celery_worker.app worker --loglevel=info -Q cyberrealistic_pony -P solo
